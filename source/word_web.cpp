...
namespace word_ladder {
	void
	word_web::init_web(const std::size_t& word_len, const absl::flat_hash_set<std::string>& lexicon) {
	...
	auto word_web::is_hamming_dist_1(const std::string& str1, const std::string& str2) -> bool {
		...
		return std::mismatch(++mismatch_pair.first, str1.end(), ++mismatch_pair.second).first
		       == str1.end();
	}
	   ...
	   -> std::vector<std::vector<std::string>> {
		...
		while (!q.empty()) {
			...
			if (path.size() != prev_ladder_len) {
				visited_prev_ladder_lens.insert(visited_curr_ladder_len.begin(),
				                                visited_curr_ladder_len.end());
		...
		ladders.erase(std::remove_if(ladders.begin(), ladders.end(), [&min](const
		std::vector<std::string>& ladder) { return ladder.size() > min;
		}), ladders.end());
		*/
		return ladders;
	}
